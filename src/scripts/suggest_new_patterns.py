...
parser = argparse.ArgumentParser( description=' Return a table with rows from one aggregate table where the key value does not occur in the second aggregate table.\nEach set of table files must be under a single branch of a directory tree.\nThe expected file format for the data is tab delimited text (tsv).\tThe column with the keys must have a header with the same name in both files.', formatter_class=argparse.RawTextHelpFormatter )
...
parser.add_argument('-g', '--generic_matches', help='Path to the directory of your first set of tsv input files.\nDefault=pattern-matches/generic_matches', nargs='?', default='pattern-matches/generic_matches', const='pattern-matches/generic_matches') # if not specified by the user then it defaults to given value
parser.add_argument('-u', '--upheno_matches', help='Path to the directory of your second set of tsv input files.\nDefault=pattern-matches/upheno_matches', nargs='?', default='pattern-matches/upheno_matches', const='pattern-matches/upheno_matches') # if not specified by the user then it defaults to given value
parser.add_argument('-k', '--keys', help='Header of column with the key values, e.g "defined_class".', nargs='?', default='defined_class', const='defined_class' ) # if not specified by the user then it defaults to given value
...
for root, dirs, files in os.walk(args.generic_matches):
    for f in files:
        if f.endswith('.tsv'):
            path2tsv = "/".join(['.', root, f]) # relative path to tsv file relative to the location of this script
    ...
    for f in files:
        if f.endswith('.tsv'):
           ...
           #print(path2tsv) # this is just for testing
            ...
            u_matches = petl.cat(u_matches, table1) # append rows from the current table to u_matches
           #c = c + 1 
...
##########################################
new_patterns = petl.antijoin(g_matches, u_matches, key=args.keys) # Return rows from the left table where the *key value* does not occur in the right table.
if args.outfile == None:
    ...
    print('Found', len(list(petl.data(new_patterns))), 'new pattern candidates.') # style options: grid, simple, minimal
