...
def obsolete_classes_from_autogenerated_upheno_tsvs(
    ...
    df_obsoleted_template = pd.read_csv(obsoleted_template)

    obsolete_classes = set(df_obsolete_fillers['cls'])
    ...
    for filename in os.listdir(tsv_directory):
        if filename.endswith(".tsv"):
            ...
            obsolete_rows = df.apply(lambda row: any(value in obsolete_classes for value in row), axis=1)
            df_obsolete = df[obsolete_rows].copy()
            df["pattern"] = pattern_name
            df["Annotation"] = df.apply(lambda row: "-".join(row.drop('defined_class').astype(str)), axis=1)
            df["GitHub Issue"] = ""
            df["Replaced"] = ""
            df["Label"] = ""
            df["Obsolete"] = "true"
            ...
            df_obsoleted_template = df_obsoleted_template.append(df_obsolete[cols], ignore_index=True)
